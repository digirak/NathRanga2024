...
\begin{document} 
   ...
   \title{Improving detection limits of brown dwarfs in spectral angular differential imaging (SADI) sequences with machine learning }
   ...
   \institute{University of Lie\`ge
...
% 5 {} token are mandatory
  ...
  % context heading (optional)
   {The advent of Integral field spectrograph (IFS) based instruments mounted on adaptive optics (AO) capable telescopes such as VLT, it has become possible to obtain data which is multi-dimensional in nature. 
   This increased dimensionality of the data produces increased computing complexity when reducing it to generate meaningful results. 
   There is a rising understanding in the community of the necessity of harnessing the power of modern algorithms and data processing techniques to make the most of this increased dimensionality. 
   An investigation, therefore, is necessary into how to use this increased dimensionality with modern machine learning algorithms to produce the highest contrast detections.}
   {It is necessary to test multiple training schemes with science appropriate reductions in order to determine the best scheme that will teach a supervised ML algorithm the features necessary to detect a faint brown dwarf.}
  % methods heading (mandatory)
  {Different methodologies exist for the use of ML in direct imaging, but none as yet have explored the multi-dimensionality of SADI data and defined an optimum training scheme which in turn can harness the predictive power of ML.
  Consequently, this work describes the different success rates of various training schemes, and the motivations to teach ML specific features that would enable it to make detections.
  This paper also discusses potential reasons why some schemes have had higher success rates than others.}
  % results heading (mandatory)
  {Results show that using the angular noise diversity allows the ML framework to discriminate effectively between $\rm{H_{0}}$ and $\rm{H_{1}}$. }
  % conclusions heading (optional), leave it empty if necessary 
{We determine that the deepest detection is achieved when a 2DCNN is exposed to data with a large diversity of contrasts of brown dwarfs.}
